...
fetch("https://emojihub.herokuapp.com/api/all", requestOptions)
    .then(response => response.json())
    .then(result => {
        console.log(result);
        console.log(result[201].htmlCode[0]);
        ...
        main.appendChild(carouselExampleCaptions);
        carouselExampleCaptions.appendChild(carouselInner);
        
        //<p id= "emoji1" class="emoji" style="font-size: 100px" >&#128512;</p>

        

        for(let i = 4; i < result.length-4; i++){
          
            let carouselItemTag = document.createElement('div');
            carouselItemTag.className="carousel-item";
            if(i == 0){
              carouselItemTag.classList.add('active');
            }
            let imgeTag = document.createElement('img');
            imgeTag.classList.add('d-block', 'w-100', 'h-25');
            imgeTag.src="./bd.jpg";
            let carouselCaption = document.createElement('div');
            carouselCaption.classList.add("carousel-caption", "d-none", "d-md-block");
            let h5Element = document.createElement('h5');
            h5Element.innerText=result[i].name;
            let emoji = document.createElement('p');
            emoji.classList.add("emoji");
            emoji.style.fontSize="50px";
            var myChar = "";
            myChar = ";";
            var thisText = unicodeConverter(result[i].htmlCode[0]);
            
            emoji.innerText=unicodeToChar = function () { 
              return thisText.replace (/\\u [dA-F] {4}/gi, function (match) { 
                return String.fromCharCode (parseInt (match.replace (/\\u/g, ''), 16)); }); 
              } ;
            
            //emoji.innerText=("&#" + unicodeConverter(result[i].htmlCode[0] + myChar));                       
         
            //console.log("&#" + unicodeConverter(result[i].htmlCode[0] + myChar));

          
            carouselInner.appendChild(carouselItemTag);
            carouselItemTag.appendChild(imgeTag);
            imgeTag.appendChild(carouselCaption);
            carouselCaption.appendChild(h5Element);
            carouselCaption.appendChild(emoji);
            
            function unicodeConverter(unicodeNumber){
              var newString = "0x";
              for(let i = 0; i < unicodeNumber.length; i++){
                  if(i > 1){                        // want to remove last character, so it's not a string
